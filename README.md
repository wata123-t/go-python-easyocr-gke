# 1.はじめに
この記事では、Go言語による高速なAPI受付と、PythonのEasyOCRエンジンをgRPCで繋ぐ、スケーラブルなOCRバックエンド基盤を構築します。単なるアプリケーションの実装に留まらず、Google Kubernetes Engine（GKE）上でのサイドカー構成を採用。Cluster Autoscaler やリソース制限（Requests/Limits）を厳密に設計することで、機械学習モデルという重量級のワークロードを、クラウドネイティブに「並列化」できる構成を目指しました。本記事のポイントは、「推論ロジック（Python）とAPI通信（Go）の責務を完全に分離し、インフラとアプリの両面から拡張性を担保した点」にあります。なお、この基盤が実際にどれほどの負荷に耐え、Podの増設によってスループットがどう向上するかについては、[後編の検証編（リンク予定箇所）] にて詳細なデータを公開しています。高負荷環境下での精度やレイテンシの推移、BigQueryを用いた分析手法に興味がある方は、併せてご参照ください。
...
ここまで、まとめてきたものは詳細仕様書の形で、別資料として置き。
設計のポイントの様な形で、以下の事を多角的に説明したいです。
まずは
...
構築したこの基盤に関する検証を**【検証編】**では行っています。
負荷試験ツール **k6** を用い、正解データ（Ground Truth）を突き合わせながら以下の項目を徹底検証します。

**・オートスケーリング:** 並列性のHPAによってPodやNodeが動的に増減する様子の可視化。
